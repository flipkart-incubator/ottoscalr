...
var _ = Describe("Deployment controller", func() {
	...
	Context("When updating the min pod annotation for a Rollout", func() {
		var rollout *rolloutv1alpha1.Rollout
		var policyreco *ottoscaleriov1alpha1.PolicyRecommendation
		now := metav1.Now()
		ctx := context.TODO()
		...
		It("Should requeue the policyreco", func() {
			By("By updating the min pod annotation for a Rollout")
			...
			createdRollout.Annotations["ottoscalr.io/min-pods"] = "50"
			Expect(k8sClient1.Update(ctx, createdRollout)).Should(Succeed())
			k8sClient1.Get(ctx,
				types.NamespacedName{Name: RolloutName, Namespace: RolloutNamespace},
				createdRollout)
	...
	Context("When updating the min pod annotation for a Deployment", func() {
		var deployment *appsv1.Deployment
		var policyreco *ottoscaleriov1alpha1.PolicyRecommendation
		ctx := context.TODO()
		now := metav1.Now()
		...
		It("Should requeue the policyreco", func() {
			...
			createdDeployment.Annotations["ottoscalr.io/min-pods"] = "50"
			Expect(k8sClient1.Update(ctx, createdDeployment)).Should(Succeed())
			updatedPolicy := &ottoscaleriov1alpha1.PolicyRecommendation{}
