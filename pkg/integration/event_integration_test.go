...
var _ = Describe("GetDesiredEvents", func() {
	It("Should return the outlier intervals by fetching the event metadata and parsing it", func() {
		...
		eventIntegration, _ = NewEventCalendarDataFetcher(server.URL, 2*time.Second, 0*time.Second, logger)
	...
	It("Should return the outlier intervals by fetching the event metadata and parsing it with event scale up buffer", func() {
		time1 := time.Now().Add(-20 * time.Minute)
		time2 := time.Now().Add(-14 * time.Minute)
		time3 := time.Now().Add(-21 * time.Minute)
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		eventIntegration, _ = NewEventCalendarDataFetcher(server.URL, 2*time.Second, 30*time.Minute, logger)
		time.Sleep(2 * time.Second)

		events, err := eventIntegration.GetDesiredEvents(time1, time2)
		Expect(err).NotTo(HaveOccurred())
		Expect(events).To(ContainElements([]EventDetails{
			{EventName: "event1", EventId: "1234", StartTime: time3.Add(-30 * time.Minute).Truncate(time.Millisecond), EndTime: time2.Truncate(time.Millisecond)},
			{EventName: "event2", EventId: "12345", StartTime: time1.Add(-30 * time.Minute).Truncate(time.Millisecond), EndTime: time2.Truncate(time.Millisecond)}}))
	})
})
