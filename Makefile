...
test: prometheus manifests generate fmt vet envtest ## Run tests with Prometheus.
	...
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" ginkgo run --cover --coverprofile cover.out ./pkg/metrics -test.v --procs 1 --nodes 1 --vv --output-interceptor-mode=none; \
    KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" ginkgo run --cover --coverprofile cover.out  --skip-file ./pkg/metrics/scraper_test.go -p 1 ./... -test.v --procs 1 --nodes 1 --vv --output-interceptor-mode=none
